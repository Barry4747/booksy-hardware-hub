...
- **Error Handling Correction:** Initially, if the SDK failed (e.g., due to an invalid model name like `models/gemini-1.5-flash`), the backend gracefully caught it and returned a `200 OK` with an empty issue array. The user pointed out this was poor UX because the frontend failed to trigger an error toast. I refactored the `except Exception` block to raise an `HTTPException(status_code=500)`, forcing the frontend Axios interceptor to catch it and notify the user via a red toast. Also updated `test_audit.py` to expect this `HTTPException`.

## UI/UX Wireframe Implementation
**Context:** Implemented light-mode UI based on a provided Figma wireframe image for `DashboardView.vue`.
- **Global Layout Changes:** Switched the dark background to `#fafafa` in `App.vue` and modified `.app-layout` to use `flex-direction: row` to support a permanent left-sidebar layout.
