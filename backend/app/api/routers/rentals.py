...
@router.get("/{id}", response_model=RentalResponse, dependencies=[Depends(get_current_user)])
def get_rental(id: int, rental_service: RentalService = Depends(get_rental_service)):
    return rental_service.get_rental(id)
